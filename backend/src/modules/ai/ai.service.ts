...
export class AiService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const primary = this.configService.get<string>('GEMINI_MODEL') || 'gemini-1.5-flash';
    const fallbackStr =
      this.configService.get<string>('GEMINI_FALLBACK_MODELS') ||
      'gemini-2.0-flash-lite,gemini-1.5-flash-8b';
    const fallbacks = fallbackStr.split(',').map((m) => m.trim()).filter(Boolean);
  ...
  async compareProducts(
    ...
    const maxTokens = this.configService.get<number>('GEMINI_MAX_TOKENS') || 4000;
    const temperature = this.configService.get<number>('GEMINI_TEMPERATURE') || 0.7;
    ...
    for (const modelName of this.modelChain) {
      try {
        const result = await this.callModel(modelName, prompt, maxTokens, temperature);
        return result;
      } catch (error) {
    ...
    this.logger.error('All Gemini models hit quota. Daily free-tier limit reached.');
  ...
  private async callModel(
    ...
    if (jsonMatch) {
      ...
      jsonString = content.replace(/^```json\s*/i, '').replace(/\s*```$/i, '').trim();
    }
    ...
    try {
      ...
      this.logger.error(`Invalid JSON from model ${modelName}. Preview: ${jsonString.slice(0, 200)}...`);
      throw new InternalServerErrorException('AI returned invalid JSON. Please try again.');
    }

    this.validateResult(result);
    this.logger.log(`✅ Comparison complete via ${modelName}. Winner: ${result.winner}`);
    return result;
  }
  ...
  private validateResult(result: ComparisonResult): void {
    ...
    for (const cat of result.categories) {
      if (
        ...
        cat.productAScore < 0 || cat.productAScore > 10 ||
        cat.productBScore < 0 || cat.productBScore > 10
      ) {
        throw new InternalServerErrorException(
